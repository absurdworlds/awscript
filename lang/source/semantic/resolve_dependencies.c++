...
struct resolver {
	...
	dependency_map& add_module_dependency(const ast::identifier& id)
	{
		auto* node = &deps;
		for (const auto& name : id.path)
		{
			node = &node->modules[name];
		}

		node = &node->modules[id.name];
